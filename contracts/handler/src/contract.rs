...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Deposit { resource_id, data } => {
            execute_deposit(deps, env, info, resource_id, &data)
        }
        ExecuteMsg::Mint { recipient, amount } => mint(deps, env, info, recipient, amount),
        ExecuteMsg::WhiteList { address } => Ok(add_to_list(deps, address, ListType::WhiteList)?),
        ExecuteMsg::BurnList { address } => Ok(add_to_list(deps, address, ListType::BurnList)?),
        ExecuteMsg::SetResourceId {
            resource_id,
            address,
        } => Ok(set_resource_id(deps, resource_id, address)?),
        ExecuteMsg::Proposal { resource_id, data } => {
            execute_proposal(deps, env, info, resource_id, &data)
        }
        ExecuteMsg::Withdraw { data } => execute_withdraw(deps, env, info, &data),
    }
}
